...
namespace BandWorld.iOS
{
    class HybridWebView : IHybridWebView
	{
		...
		public void SetMVCManager(MVCManager mvcManager)
		{
			...
			// Register interop object with MVCManager.
			mvcManager.RegisterNativeObject("JavaScriptInterop", interop);
		}
		...
		public string EvaluateJavascript(string script)
		{
			...
			webView.EvaluateJavascript(currentUrl);
			return "";
		}
		...
		public bool HandleRequest(string testUrl)
		{
			...
			if (inLoadCount == 0)
			{
				handled = MVCManager.ShouldOverrideUrlLoading(url);
