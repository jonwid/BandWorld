...
namespace JTRazorPortable
{
    ...
    public class MVCManager
    {
        ...
        public Dictionary<string, object> NativeObjects { get; private set; }
        ...
        public MVCManager(IHybridWebView hybridView)
        {
            ...
            NativeObjects = new Dictionary<string, object>(StringComparer.OrdinalIgnoreCase);
        ...
        public void RegisterNativeObject(string objectName, object nativeObject)
        {
            if (!NativeObjects.ContainsKey(objectName))
                NativeObjects.Add(objectName, nativeObject);
        }

        public object FindNativeObject(string objectName)
        {
            object nativeObject = null;

            if (!NativeObjects.TryGetValue(objectName, out nativeObject))
                nativeObject = null;
        ...
        public bool ShouldOverrideUrlLoading(string url)
        {
            ...
            {
                string ajaxObjectJson = url.Substring(5);
                ajaxObjectJson = QueryValueDecode(ajaxObjectJson);
                string jsonReturn = HandleAjaxRequest(ajaxObjectJson);
                ajaxObjectJson = JavascriptEncode(ajaxObjectJson);
                jsonReturn = JavascriptEncode(jsonReturn);
                string returnCall = "PortableAjaxReturn('" + ajaxObjectJson + "', '" + jsonReturn + "')";
                HybridView.EvaluateJavascript(returnCall);
                return true;
            }
            else if (url.StartsWith("call:"))
            {
                ...
                object returnValue;
                if (HandleNativeCall(url, out returnCallback, out returnValue))
                {
                    if (!String.IsNullOrEmpty(returnCallback))
                    {
                        string returnValueString = returnValue.ToString();
                        returnValueString = MVCManager.JavascriptEncode(returnValueString);
                        string returnCall;
                        if (returnValue is string)
                            returnCall = returnCallback + "('" + returnValueString + "')";
                        else
                            returnCall = returnCallback + "(" + returnValueString + ")";
                        HybridView.EvaluateJavascript(returnCall);
                    }
        ...
        public bool HandleNativeCall(string url, out string returnCallback, out object returnValue)
        {
            object nativeObject;
            string functionName;
            MethodInfo method;
            ...
            if (ProcessNativeCallUrl(url, out nativeObject, out functionName, out method, out paramsIn,
                out returnCallback))
            {
                try
                {
                    // Invoke controller function
                    returnValue = method.Invoke(nativeObject, paramsIn);
                }
                catch (Exception)
                {
                    return false;
        ...
        public bool ProcessNativeCallUrl(
            string url,
            out object nativeObject,
            out string functionName,
            out MethodInfo method,
            out object[] paramsIn,
            out string returnCallback)
        {
            string queryString;
            ...
            if (ProcessNativeCallObjectAndMethod(
                    url,
                    out nativeObject,
                    out functionName,
                    out queryString,
                    out method))
            {
                if (!ProcessNativeCallArguments(method, queryString, out paramsIn, out returnCallback))
                    return false;
            }
            else
                return false;
        ...
        public bool ProcessNativeCallObjectAndMethod(
            string url,
            out object nativeObject,
            out string functionName,
            out string queryString,
            out MethodInfo method)
        {
            ...
            if (queryIndex >= 0)
            {
                functionName = path.Substring(0, queryIndex);
                queryString = path.Substring(queryIndex + 1);
            }
            else
                functionName = path;

            string objectName = "JavaScriptInterop";

            if (functionName.Contains("/"))
            {
                var parts = functionName.Split(pathSeps, StringSplitOptions.RemoveEmptyEntries);
                objectName = parts[0];
            ...
            if (!NativeObjects.TryGetValue(objectName, out nativeObject))
            {
                HandleError("Native object not registered: " + objectName);
                return false;
            }

            method = nativeObject.GetType().GetRuntimeMethod(functionName);

            if (method == null)
            {
                HandleError("Can't find native object function " + functionName);
                return false;
            }
        ...
        public bool ProcessNativeCallArguments(
            MethodInfo method,
            string queryString,
            out object[] paramsIn,
            out string returnCallback)
        {
            ...
            returnCallback = parameters.GetOrDefault("returnCallback", "");
        ...
        {
            return HttpUtility.JavascriptEncode(str);
        }

        public static string QueryValueDecode(string str)
        {
            return HttpUtility.UrlDecode(str);
        }

        public static string QueryValueEncode(string str)
        {
