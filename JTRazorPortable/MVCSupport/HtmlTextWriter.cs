...
namespace JTRazorPortable
{
    ...
    public class HtmlTextWriter : TextWriter
    {
        ...
        private TextWriter _httpWriter;
        ...
        */
...
#else
                throw new Exception("PopEndTag doesn't match a push.");
#endif
            }
            _endTagCount--;
...
#else
            throw new Exception("HtmlTextWriter.WriteUTF8ResourceString not implemented.");
#endif
        }
        ...
        protected void WriteUrlEncodedString(String text, bool argument)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    Write('%');
                    Write(HttpUtility.IntToHex((ch >> 4) & 0xf));
                    Write(HttpUtility.IntToHex((ch) & 0xf));
                }
                else
                {
        ...
        internal void WriteHtmlAttributeEncode(string s)
        {
            ...
            {
                HttpUtility.HtmlAttributeEncodeInternal(s, _httpWriter);
            }
        }
